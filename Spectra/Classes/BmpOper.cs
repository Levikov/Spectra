...
namespace Spectra
{
    class BmpOper
    {
        ...
        /// <param name="low">亮度等级1-8(4为原始图像)</param>
        /// <returns></returns>
        public static Task<Bitmap> MakePseudoColor2(string path, UInt16[] band, UInt16 gain)
        {
            return Task.Run(() => {
                if (band[0] > 160 || band[1] > 160 || band[2] > 160 || gain > 8)
                    return null;
                ...
                else
                {
                    for (int i = 0; i < 3; i++)
                    {
                        for (int j = 0; j < splitSum; j++)
                        {
                            ...
                            try
                            {
                                fBmp[i] = new FileStream($"{path}{j}\\{band[i]}.raw", FileMode.Open, FileAccess.Read, FileShare.Read);
                                byte[] tBuf = new byte[splitHeight * width * 2];
                                fBmp[i].Read(tBuf, 0, width * splitHeight * 2);
                                fBmp[i].Close();
                                Array.Copy(tBuf, 0, bufBand[i], i * 4096 * 2048 * 2, width * splitHeight * 2);
                            }
                            catch { }
                        }
                    }
